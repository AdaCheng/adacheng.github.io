...
首先，将每个词转换为 One-hot 的形式作为输入，存在共享矩阵 $C$（大小为 $\vert V \vert \times m$，其中 $\vert V \vert$ 表示词表的大小，$m$ 表示词向量的维度）作为 look-up table，通过从矩阵中取出一行，将词 $w$ 的 One-hot 表示转换为 $C(w)$，即词 $w$ 所对应的词向量。

然后，再将词向量 $C(w_{t-n+1}, ..., C(w_{t-2}), C(w_{t-1}))$ 首尾拼接，形成长度为 $(n-1) \times m$的向量，记作 $x$。
将 $x$ 进行线性变换并把 tanh 作为激活函数。
