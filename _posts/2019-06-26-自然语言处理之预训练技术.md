...
$$\sum_{k=1}^{N}\left(\log p\left(t_{k} | t_{1}, \ldots, t_{k-1} ; \Theta_{x}, \vec{\Theta}_{L S T M}, \Theta_{s}\right)\right+\log p\left(t_{k} | t_{k+1}, \ldots, t_{N} ; \Theta_{x}, \Theta_{L S T M}, \Theta_{s}\right) )$$

第二阶段，将任务相关的语料输入已训练完成的网络中，每个单词都能从模型每层提取到与之对应的不同信息层次的 Embeddings：
- Word Embedding；
- Syntax Embedding；
- Semantics Embedding。

将三个 Embeddings 按照学习到的权重 $\gama$ 组合后，整合成为一个向量 $ELMo_k^{task}$:

$$
{ELMo}_{k}^{task}=E\left(R_{k} ; \Theta^{t a s k}\right)=\gamma^{\operatorname{task}} \sum_{j=0}^{L} s_{j}^{t a s k} \mathbf{h}_{k, j}^{L M}
$$

将向量 $ELMo_k^{task}$ 与上下文无关向量 $x_k$ 进行拼接后，作为输入传入下游模型中。 
