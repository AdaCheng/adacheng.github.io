...
自然语言处理中早期的预训练模型是 Word Embedding 技术，将词汇表中的单词或者短语映射到向量空间中，用以解决 One-hot 等技术所面临的稀疏性高、捕捉信息不足等缺点。
...
在预训练模型之前，必须先简单地提一下神经概率语言模型，一是因为早期表现出色的 Word2Vec 的灵感来源其中，二是因为目前 NLP 里面大部分预训练都是用语言模型任务来完成。

在此，默认大家都具备**统计语言模型**的相关知识，
