...
$$
\begin{array}{l}{\sum_{k=1}^{N}\left(\log p\left(t_{k} | t_{1}, \ldots, t_{k-1} ; \Theta_{x}, \overrightarrow{\Theta}_{LSTM}, \Theta_{s}\right)\right+\log p\left(t_{k} | t_{k+1}, \ldots, t_{N} ; \Theta_{x}, \overleftarrow{\Theta}_{LSTM}, \Theta_{s}\right) )}\end{array}
$$

第二阶段，将任务相关的语料输入已训练完成的网络中，每个单词 $t_k$ 都能从模型每层提取到与之对应的不同信息层次的 Embeddings：
- 输入层的 Word Embedding，$h_{k,0}^{LM}$；
- $\overrightarrow{h}$
- 第一层双向 LSTM 对应的 Syntax Embedding，$\overrightarrow{h}_{k,1}^{LM};\ovreleftarrow{h}_{k,1}^{LM}$；
- 第二层双向 LSTM 对应的 Semantics Embedding $[\overrightarrow{h}_{k,2}^{LM};\ovreleftarrow{h}_{k,2}^{LM}]$。
