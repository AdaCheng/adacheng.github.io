...
该论文由 Begio 等人于 2003 年发表在 JMLR 上，文中提出了一个**神经网络语言模型**（以下简称 NNLM），与 N-gram 一样，同样假设当前词出现的概率只与前 $k$ 个词语相关。

![NNLM](/assets/images/post/2019-06-26/NNLM-1.png)

为了便于理解，将神经网络语言模型可以分为四层，即输入层、embedding层、隐藏层、输出层。

任务描述：
    从语料库中随机选取长度为 $n$ 的文本，即图中 $w_{t-n+1}, ..., w_{t-2}, w_{t-1}$，根据这前 $n-1$ 个词预测下一个词 $w_{t}$。


首先，将 input 转换为 One-hot 的形式作为输入，存在共享矩阵 $C$（大小为 $\vert V \vert \times m$，其中 $\vert V \vert$ 表示词表的大小，$m$ 表示词向量的维度）作为 look-up table，通过从矩阵中取出对应的一行，将词 $w$ 的 One-hot 表示转换为 $C(w)$，即词 $w$ 所对应的词向量。

然后，再将词向量 $C(w_{t-n+1}), ..., C(w_{t-2}), C(w_{t-1})$ 首尾拼接，形成长度为 $(n-1) \times m$的向量，记作 $x$。

通过隐藏层与直连边，得到输出层 $\vert V \vert$ 个节点，其中每个节点 $y_i$ 表示下一个词为 $i$ 的未归一化对数概率，

$$ y = b + Wx + U \tanh (d+Hx) $$

其中，$h$ 为隐藏单元个数，$H$（大小为 $h \times (n-1)m$）是输入层到隐藏层的参数，$U$（大小为 $\vert V \vert \times h$） 是隐藏层到输出层的参数，$W$（大小为 $\vert V \vert \times (n-1)m$）是输入层到输出层的直连边。
