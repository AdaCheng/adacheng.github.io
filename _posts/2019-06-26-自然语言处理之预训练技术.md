...
- 第一层双向 LSTM 对应的 Syntax Embedding，$\left[\overrightarrow{\mathbf{h}}_{k, j}^{L M} ; \overleftarrow{\mathbf{h}}_{k, j}^{L M}\right]$；
- 第二层双向 LSTM 对应的 Semantics Embedding，$[\overrightarrow{h}_{k,2}^{L M} ; \overleftarrow{h}_{k,2}^{LM}]$。
