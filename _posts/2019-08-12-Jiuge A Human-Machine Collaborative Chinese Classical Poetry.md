...
- For history memory, select a slot by writing [addressing function](#AF) and fill encoder state $h_t$ of $L_{i-2}$ into it.
    ...
    where $\alpha_w$ is the writing probabilities vector, $\tilde{M}_2$ is the concatenation of history memory $M_2$ and a null slot, $v_{i-1}$ is [global trace vector](#GTV).
